...
void cliqueNormal(){
	...
	for(int verticeCorrente = 0; verticeCorrente<qtd_vertices;++verticeCorrente){
		...
		for(j = 0; j<qtd_palavrasULL;++j) //pode melhorar, pra que isso se toda a linha for zero?
			R(i,j) = G(verticeCorrente,j);



		c[palavraRaiz] = c[palavraRaiz] | (1ULL<<verticeCorrente);
		...
		for(int k = 0; k<qtd_vertices;++k)
			G(k,palavraRaiz) = G(k,palavraRaiz) & ~(1ULL<<verticeCorrente);
